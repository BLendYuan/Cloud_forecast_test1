...
public class DBManager {
    ...
    //存储城市天气，要求最多存储5个城市信息，超过5个就不能存储，获取目前已经存储的数量
    ...
    public static List<DatabaseBean> queryAllInfo(){
        ...
        while (cursor.moveToNext()){
            ...
            String city = cursor.getString(cursor.getColumnIndex("city"));
            String contrnt = cursor.getString(cursor.getColumnIndex("content"));
            DatabaseBean bean = new DatabaseBean(id,city,contrnt);
            list.add(bean);
        }
        return list;
    }
